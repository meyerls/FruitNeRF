...
import json
data_path = "/home/se86kimy/Dropbox/05_productive/01_code/07_NeRF/14_PagNerf/pagnerf/datasets/CKA_sweet_pepper_2020_summer/20200924/row1/ns/transforms.json"
f = open(data_path)
data = json.load(f)
for idx, frame in enumerate(data['frames']):
    frame.update({'semantic_path': frame['file_path'].replace('images', 'semantics')})
    data['frames'][idx] = frame
...
import json
data_path = "/home/se86kimy/Dropbox/05_productive/01_code/07_NeRF/14_PagNerf/pagnerf/datasets/CKA_sweet_pepper_2020_summer/20201001/row5/ns/transforms.json"
f = open(data_path)
data = json.load(f)
sort_frames = {}

for i in range(len(data['frames'])):
    sort_frames.update({i: None}) 

for idx, frame in enumerate(data['frames']):
    frame.update({'semantic_path': frame['file_path'].replace('images', 'semantics')})
    data['frames'][idx] = frame
    sort_frames[frame['colmap_im_id']] = frame

l = []
for idx in sort_frames:
    l.append(sort_frames[idx])
